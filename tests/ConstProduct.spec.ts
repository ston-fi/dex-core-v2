...
import { createMdGraphLocal, dumpRawCells, getLpAccDataNoFail, GraphParamsWithoutPath, preprocBuildContractsLocal } from '../helpers/helpers';
import { AddressMap, buildLibFromCell, buildLibs, DAY_IN_SECONDS, DEFAULT_JETTON_MINTER_CODE, DEFAULT_JETTON_WALLET_CODE, divUp, emptyCell, getWalletBalance, HOLE_ADDRESS, JettonContent, JettonMinterContract, jettonMinterStorageParser, JettonWalletContract, jettonWalletStorageParser, maxBigint, metadataCell, onchainMetadata, stdFtOpCodes } from '../libs';
...
import { PoolCPI as Pool, poolCpiStorageParser } from '../wrappers/Pool';
import { crossSwapPayload, provideLpPayload, Router, routerStorageParser, swapPayload } from '../wrappers/Router';
...
type SBCtrVault = SandboxContract<Vault>;
...
type CrossRouterSwapParams = {
    ...
    excessesReceiver?: SBCtrTreasury,
    amountIn: bigint,
    debugGraph?: string,
    expectBounce?: boolean,
...
describe('Const Product', () => {
    ...
    let code: { router: Cell; lpWallet: Cell; lpAccount: Cell; pool: Cell; vault: Cell; poolDummy: Cell; },
    ...
    beforeAll(async () => {
        ...
        if (process.env.DUMP_RAW_CELLS) {
            dumpRawCells(_code, "build/cpi_raw_cells.json", "base64")
        }
        ...
        createPool = async (params: CreatePoolParams) => {
            ...
            storageMap.set(pool, poolCpiStorageParser);
        ...
        swap = async (params: SwapParams) => {
            ...
            if (params.slippage) {
                const isLeft = routerWalletIn.address.equals(oldPoolData.leftJettonAddress);
                const maxOut = (isLeft ? oldPoolData.rightReserve * params.amountIn / oldPoolData.leftReserve : oldPoolData.leftReserve * params.amountIn / oldPoolData.rightReserve);
                // @ts-ignore
                params.minAmountOut = maxOut * BigInt(10000 - params.slippage) / 10000n;
            } else {
            ...
            let msgResult = await walletIn.sendTransfer(sender.getSender(), {
                ...
                fwdPayload: swapPayload({
                    ...
                    deadline: params.txDeadline ?? initTimestamp + HOUR_IN_SECONDS
                }),
            });
            if (params.debugGraph) {
                createMdGraphWithPath({
    ...
    describe('Pool code upgrade', () => {

        it('should init new pool code', async () => {
            ...
            const jetton1 = setup.token1
            const jetton2 = setup.token2
            const pool = setup.pool as SBCtrPool

            let wallet1 = await jetton1.getWalletAddress(router.address)
            let wallet2 = await jetton2.getWalletAddress(router.address)
            let msgResult = await router.sendInitPoolCodeUpgrade(deployer.getSender(), {
                newCode: code.poolDummy
            })
            createMdGraphWithPath({
                msgResult: msgResult,
                storageMap: storageMap,
                addressMap: addressMap,
                bracketMap: bracketMap,
                output: "pool_code_init",
                chartType: "LR",
            });
            expectNotBounced(msgResult.events)
            const data = await router.getRouterData()
            expect(data.endPool).toBeGreaterThan(0n)
            expect(data.pendingPoolCode.equals(code.poolDummy)).toBeTruthy()
        });

        it('should cancel new pool code', async () => {
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000),
                    amount2: toNano(2000),
            ...
            expectNotBounced(msgResult.events)
            msgResult = await router.sendCancelPoolCodeUpgrade(deployer.getSender())
            createMdGraphWithPath({
                msgResult: msgResult,
                storageMap: storageMap,
                addressMap: addressMap,
                bracketMap: bracketMap,
                output: "pool_code_cancel",
                chartType: "LR",
            });
            expectNotBounced(msgResult.events)

            const data = await router.getRouterData()
            expect(data.endPool).toEqual(0n)
            expect(data.pendingPoolCode.equals(emptyCell())).toBeTruthy()
        });

        it('should finalize new pool code', async () => {
            setFromInitTimestamp(10)
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000),
                    amount2: toNano(2000),
            ...
            let wallet1 = await jetton1.getWalletAddress(router.address)
            let wallet2 = await jetton2.getWalletAddress(router.address)
            advanceFromCurrentTS(10)
            let msgResult = await router.sendInitPoolCodeUpgrade(deployer.getSender(), {
                newCode: code.poolDummy
            })
            expectNotBounced(msgResult.events)
            advanceFromCurrentTS(DAY_IN_SECONDS * 2 + 10)
            msgResult = await router.sendFinalizeUpgrades(deployer.getSender())
            createMdGraphWithPath({
                msgResult: msgResult,
                storageMap: storageMap,
                addressMap: addressMap,
                bracketMap: bracketMap,
                output: "pool_code_finish",
                chartType: "LR",
            });
            ...
            const updPoolCode = await router.getUpgradedPoolCode()
            expect(updPoolCode.equals(code.poolDummy)).toBeTruthy()
        });

        it('should not finalize new pool code if time not passed', async () => {
            setFromInitTimestamp(10)
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000),
            ...
            const updPoolCode = await router.getUpgradedPoolCode()
            expect(updPoolCode.equals(emptyCell())).toBeTruthy()
        });

        it('should set new pool code on pool', async () => {
            setFromInitTimestamp(10)
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000),
            ...
            advanceFromCurrentTS(100)
            msgResult = await router.sendUpgradePoolCode(deployer.getSender(), {
                firstWalletAddress: wallet1,
                secondWalletAddress: wallet2
            })
            createMdGraphWithPath({
                msgResult: msgResult,
                storageMap: storageMap,
                addressMap: addressMap,
                bracketMap: bracketMap,
                output: "pool_code_upgrade_pool",
                chartType: "LR",
            });
            expectNotBounced(msgResult.events)

            const data = await pool.getPoolData()
            expect(data.totalSupplyLP).toEqual(toNano(1337))

            msgResult = await router.sendUpdatePoolStatus(deployer.getSender(), {
                firstWalletAddress: wallet1,
                secondWalletAddress: wallet2
            })
            createMdGraphWithPath({
                ...
                addressMap: addressMap,
                bracketMap: bracketMap,
                output: "pool_code_dummy_bounce",
                chartType: "LR",
            });
            expectBounced(msgResult.events)
        });

        it('should not set new pool code if empty', async () => {
            setFromInitTimestamp(10)
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000),
            ...
            let msgResult = await router.sendUpgradePoolCode(deployer.getSender(), {
                firstWalletAddress: wallet1,
                secondWalletAddress: wallet2
            })
            createMdGraphWithPath({
                ...
                addressMap: addressMap,
                bracketMap: bracketMap,
                output: "pool_code_upgrade_bounce",
                chartType: "LR",
            });
            expectBounced(msgResult.events)
    ...
    describe('Dex', () => {
        ...
        it('should provide lp with payload', async () => {
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000),
                    amount2: toNano(1000),
                    debugGraph: "create_provide_single_payload_ssp",
                }
            });
            const aliceWallet1 = await getWalletContract(bc, setup.token1, alice);
            const aliceWallet2 = await getWalletContract(bc, setup.token2, alice);
            const routerWallet1 = await getWalletContract(bc, setup.token1, setup.router.address);
            const routerWallet2 = await getWalletContract(bc, setup.token2, setup.router.address);

            let msgResult1 = await aliceWallet1.sendTransfer(alice.getSender(), {
                value: toNano(2),
                ...
                fwdPayload: provideLpPayload({
                    ...
                    excessesAddress: alice.address,
                    toAddress: alice.address,
                    deadline: initTimestamp + HOUR_IN_SECONDS
                })
            });

            let msgResult2 = await aliceWallet2.sendTransfer(alice.getSender(), {
                value: toNano(2),
                jettonAmount: toNano("1"),
                toAddress: setup.router.address,
                responseAddress: alice.address,
            ...
            expect(msgResult2.transactions).toHaveTransaction({
                to: alice.address,
                op: stdFtOpCodes.ftTransferNotification,
                body(x) {
                    const body = x?.asSlice();
                    const payload = body?.loadRef().asSlice();
                    return payload?.loadUint(32) == 10;
                },
            })

            createMdGraphWithPath({
                msgResult: msgResult1,
                storageMap: storageMap,
                addressMap: addressMap,
                bracketMap: bracketMap,
                output: "provide_lp_payload1",
                chartType: "LR"
            });
            ...
            createMdGraphWithPath({
                ...
                addressMap: addressMap,
                bracketMap: bracketMap,
                output: "provide_lp_payload2",
                chartType: "LR"
            });
        });

        it('should provide lp with payload to a different address', async () => {
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000),
                    amount2: toNano(1000),
        ...
        it('should handle more complex scenarios', async () => {
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000),
                    amount2: toNano(1000),
            ...
            let jetton4 = await deployJetton({
                name: "token4",
                router: setup.router,
                mintAmount: toNano(100000)
            });

            let pool2 = await createPool({
                router: setup.router,
                token1: setup.token2,
                token2: jetton3,
                amount1: toNano(1000),
            ...
            const routerWallet1 = await getWalletContract(bc, setup.token1, setup.router.address);
            const routerWallet2 = await getWalletContract(bc, setup.token2, setup.router.address);
            const routerWallet3 = await getWalletContract(bc, jetton3, setup.router.address);
            const routerWallet4 = await getWalletContract(bc, jetton4, setup.router.address);

            let msgResult2 = await aliceWallet1.sendTransfer(alice.getSender(), {
                value: toNano(2),
                jettonAmount: toNano("1"),
                toAddress: setup.router.address,
                responseAddress: alice.address,
                fwdAmount: toNano("1"),
                fwdPayload: swapPayload({
                    otherTokenWallet: routerWallet2.address,
                    receiver: alice.address,
                    refundAddress: alice.address,
                    excessesAddress: alice.address,
                    deadline: initTimestamp + HOUR_IN_SECONDS,
                    customPayload: crossSwapPayload({
                        otherTokenWallet: routerWallet3.address,
            ...
            createMdGraphWithPath({
                ...
                addressMap: addressMap,
                bracketMap: bracketMap,
                output: "cross_swap_payload_complex",
                chartType: "LR"
            });
            ...
            expect(msgResult2.transactions).toHaveTransaction({
                ...
                body: (x) => {
                    ...
                    return ((jettonAmount != undefined) && (jettonAmount > 0n)) && ((fromAddress != undefined));
                },
            })
        });
        ...
        it('should provide lp (single side)', async () => {
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000) * toNano(1),
                    amount2: toNano(1000) * toNano(1),
                }
            });
        ...
        it('should burn lp and swap', async () => {
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000) * toNano(1),
                    amount2: toNano(1000) * toNano(1),
            ...
            let pool = await provideLp({
                ...
                amount1: toNano(10),
                amount2: toNano(0),
                debugGraph: "provide_single_burn",
                minLpOut: 1n
            });
            ...
            let msgResult = await lpWallet.sendBurnExt(alice.getSender(), {
                jettonAmount: oldBalance,
                customPayload: {
                    leftPayload: crossSwapPayload({
                        otherTokenWallet: poolData.rightJettonAddress,
                        receiver: alice.address,
                        excessesAddress: alice.address,
                        refundAddress: alice.address,
                        deadline: initTimestamp + HOUR_IN_SECONDS
        ...
        it('should provide lp (single side) with protocol fees', async () => {
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000) * toNano(1),
                    amount2: toNano(3000) * toNano(1),
                }
            });
            ...
            let pool = await provideLp({
                ...
                amount1: toNano(1499.5),
                amount2: toNano(0),
                debugGraph: "provide_single_protocol_fee",
                minLpOut: 1n,
            });
        ...
        it('should refund swap if tx is expired', async () => {
            ...
            advanceFromCurrentTS(60 * 15); // + 15 min
            ...
            await swap({
                ...
                tokenOut: setup.token2,
                amountIn: toNano(2),
                debugGraph: "swap_refund_tx_expired",
                txDeadline: initTimestamp + 60 * 10,
                expectRefund: true
            });
        });
    ...
    describe('Refund', () => {
        ...
        it('should refund both lp tokens', async () => {
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000),
                    amount2: toNano(2000),
