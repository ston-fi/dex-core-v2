...
import { AddressMap, DEFAULT_JETTON_MINTER_CODE, DEFAULT_JETTON_WALLET_CODE, HOLE_ADDRESS, JettonContent, JettonMinterContract, JettonWalletContract, buildLibFromCell, buildLibs, getWalletBalance, jettonMinterStorageParser, jettonWalletStorageParser, maxBigint, metadataCell, onchainMetadata } from '../libs';
...
import { PoolStable as Pool, poolStableStorageParser } from '../wrappers/Pool';
import { RouterSI as Router, crossSwapPayload, provideLpPayload, routerStorageParser, swapPayload } from '../wrappers/Router';
...
describe('Stableswap', () => {
    ...
    beforeAll(async () => {
        ...
        if (process.env.DUMP_RAW_CELLS) {
            dumpRawCells(_code, "build/stableswap_raw_cells.json", "base64")
        }
        ...
        createPool = async (params: CreatePoolParams) => {
            ...
            let lockPool = params.lockPool ?? false;
            ...
            storageMap.set(pool, poolStableStorageParser);
        ...
        provideLp = async (params: ProvideLpParams) => {
            ...
            if (params.amount1) {
                let msgResult = await wallet1.sendTransfer(sender.getSender(), {
                    ...
                    fwdPayload: provideLpPayload({
                        otherTokenAddress: routerWallet2.address,
                        minLpOut: shouldMint ? 0n : params.minLpOut,
                        bothPositive: shouldMint ? true : false,
                        toAddress: sender.address,
                        refundAddress: sender.address,
                        deadline: initTimestamp + HOUR_IN_SECONDS
                    })
            ...
            if (shouldMint && !params.expectRevert) {
                let poolData = await pool.getPoolData();
        ...
        setPoolParams = async (params: SetPoolParams) => {
            params.newAmp = params.newAmp ?? 1n;
            let router = params.router;
            let sender = params.sender ?? deployer;
            let routerWallet1 = await getWalletContract(bc, params.token1, router.address);
            let routerWallet2 = await getWalletContract(bc, params.token2, router.address);
    ...
    describe('Params', () => {
        ...
        it('should set param', async () => {
            ...
            let data2 = await (setup.pool as SBCtrPool).getPoolData();
            expect(data2.amp).toBe(10n);
        });
    ...
    describe('Dex', () => {
        it('should deploy dex', async () => {
            ...
            expect(data.type).toEqual("stableswap");
            let poolData = await setup.pool?.getPoolType();
            expect(poolData).toEqual("stableswap");
        });
        ...
        it('should provide lp (single side)', async () => {
            ...
            let pool = await provideLp({
                ...
                sender: alice,
                amount1: toNano(100),
                debugGraph: "provide_single_wcpi1",
                minLpOut: 1n
            });
        ...
        it('should swap with different amp', async () => {
            let setup = await setupDex({
                createPool: {
