...
import { PoolCWSI as Pool, poolCwsiStorageParser } from '../wrappers/Pool';
import { RouterCWSI as Router, crossSwapPayload, provideLpPayload, routerStorageParser, swapPayload } from '../wrappers/Router';
...
describe('Weighted Stableswap', () => {
    ...
    beforeAll(async () => {
        ...
        if (process.env.DUMP_RAW_CELLS) {
            dumpRawCells(_code, "build/wstableswap_raw_cells.json", "base64")
        }
        ...
        createPool = async (params: CreatePoolParams) => {
            ...
            storageMap.set(pool, poolCwsiStorageParser);
            ...
            if (defaultParams) {
                let setParam = await setPoolParams({
                    newAmp: params.amp,
                    router: router,
                    token1: params.token1,
                    token2: params.token2,
        ...
        setPoolParams = async (params: SetPoolParams) => {
            ...
            params.newRate = params.newRate ?? 0n;
            params.newAmp = params.newAmp ?? 1000000000000000000n;
            let router = params.router;
            let sender = params.sender ?? deployer;
            let routerWallet1 = await getWalletContract(bc, params.token1, router.address);
            let routerWallet2 = await getWalletContract(bc, params.token2, router.address);
            ...
            let msgResult = await router.sendSetParams(sender.getSender(), {
                leftWalletAddress: routerWallet1.address,
                rightWalletAddress: routerWallet2.address,
                newAmp: params.newAmp,
                newRate: params.newRate,
                newW: params.newW,
                sideWalletAddress: sideWallet,
                setterAddress: params.setter?.address
            }, params.gas ?? toNano(2));
            if (params.debugGraph) {
                createMdGraphWithPath({
                    msgResult: msgResult,
            ...
            if (params.expectBounce) {
                ...
                expectNotBounced(msgResult.events);
                let poolData = await pool.getPoolData();
                expect(poolData.amp).toEqual(params.newAmp);
                if (params.newRate)
                    expect(poolData.rate).toEqual(params.newRate);
                expect(poolData.w0).toEqual(sideWallet.equals(poolData.leftJettonAddress) ? params.newW : ONE_DEC - params.newW);
                if (params.setter)
                    expectEqAddress(params.setter, poolData.setterAddress);
            }
    ...
    describe('Params', () => {
        ...
        it('should set params (no setter)', async () => {
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000000),
                    amount2: toNano(1000000),
            ...
            expectEqAddress(data.setterAddress, null);
        ...
        it('should swap with different ratio', async () => {
            ...
            await setPoolParams({
                ...setup,
                newAmp: 2000000000000000000n,
                newRate: 1000000000000000000n,
                newW: 800000000000000000n,
                sideToken: setup.token1,
            });
        ...
        it('should set new rate with setter call', async () => {
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000000),
                    amount2: toNano(1000000),
            ...
            let pool = setup.pool as SBCtrPool;
            let data = await pool.getPoolData();
            await setPoolParams({
                ...setup,
                newAmp: 3000000000000000000n,
                newW: 800000000000000000n,
                sideToken: setup.token1,
                setter: alice,
            });

            let msgResult = await pool.sendSetRate(alice.getSender(), {
                newRate: 1000000000000000000n
            });
            createMdGraphWithPath({
                msgResult: msgResult,
            ...
            expectNotBounced(msgResult.events);
            data = await pool.getPoolData();
            expect(data.rate).toEqual(1000000000000000000n);
            expect(data.amp).toEqual(3000000000000000000n);
        });
    });
    ...
    describe('Dex', () => {
        it('should deploy dex', async () => {
            ...
            expect(data.type).toEqual("weighted_stableswap");
            let poolData = await setup.pool?.getPoolType();
            expect(poolData).toEqual("weighted_stableswap");
        });
    ...
    describe('Bounce', () => {
        ...
        it('should bounce set new rate by non-setter', async () => {
            let setup = await setupDex({
                createPool: {
                    amount1: toNano(1000000),
                    amount2: toNano(1000000),
            ...
            await setPoolParams({
                ...setup,
                newAmp: 8000000000000000000n,
                newW: 800000000000000000n,
                sideToken: setup.token1,
                setter: alice,
            });

            let msgResult = await pool.sendSetRate(bob.getSender(), {
                newRate: 1000000000000000000n
            });
            createMdGraphWithPath({
                msgResult: msgResult,
